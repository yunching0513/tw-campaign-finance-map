# 只允許台灣 IP ＋ 擋機器人／限流（Cloudflare 設定指引）

GitHub Pages 是純靜態託管，**無法**依 IP 封鎖、限流或擋機器人。要真正做到，得在網站前面加一層
**Cloudflare（免費方案即可）**。本指引以你們的網域 `visionzero.tw` 開一個子網域為例。

> ⚠️ 重要前提：GitHub 的 `yunching0513.github.io` 原始網址**永遠公開**，就算前面擋了，有人直接打
> github.io 仍進得去。要徹底只走台灣，請見最後「徹底封鎖」一節（改用 Cloudflare Pages 託管，
...
- Rule name：`Taiwan only`
- 條件（Edit expression）：
  ```
  (ip.geoip.country ne "TW")
  ```
- Action：**Block**（或 Managed Challenge 給可疑流量驗證）
- Deploy。
> 之後只有台灣 IP 能進 `money.visionzero.tw`。
...
- 前端「台灣軟閘」：非台灣 IP 顯示提示頁（用 `api.country.is` 判斷國別）。
  **可被繞過、非真防護**，且會把訪客 IP 送給第三方定位服務——真正的封鎖請用上面的 Cloudflare。
